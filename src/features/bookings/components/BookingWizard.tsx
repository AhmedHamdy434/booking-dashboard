      ...
      {[1, 2, 3].map((i) => (
        <div key={i} className="flex flex-col items-center">
          ...
          <span className={`text-xs mt-2 ${step === i ? 'text-foreground font-medium' : 'text-muted-foreground'}`}>
            {i === 1 ? 'Service' : i === 2 ? 'Barber' : 'Time'}
          </span>
        </div>
      ))}
      <div className="absolute left-1/2 -translate-x-1/2 top-4 w-64 h-0.5 bg-muted -z-10">
        <div 
                  ...
                  >
                    <CardContent className="p-6 flex flex-col justify-between h-full">
                      ...
                      <div className="flex items-center justify-between mt-4 pt-4 border-t border-border/50">
                        <span className="font-bold text-lg">{service.price} EGP</span>
        ...
        {step === 2 && (
          <div className="space-y-4">
            <div className="flex items-center mb-6">
              ...
              <h2 className="text-2xl font-bold flex-1 text-center pr-12">Select a Barber</h2>
            </div>
            {isLoadingBarbers ? (
              <div className="flex justify-center p-12"><Loader2 className="w-8 h-8 animate-spin text-primary" /></div>
            ) : (
        ...
        {step === 3 && (
          <div className="space-y-6">
            <div className="flex items-center mb-6">
              ...
              <h2 className="text-2xl font-bold flex-1 text-center pr-12">Select Date & Time</h2>
            </div>
                ...
                <div className="grid gap-6 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-2">
                      Enter the time you want to start your appointment.
                    </p>
                  </div>
                </div>
                  ...
                  {currentDayAvailability ? (
                    <div className="p-4 bg-green-50 dark:bg-green-900/10 border border-green-100 dark:border-green-900/30 rounded-lg text-sm">
                      ...
                      <p className="text-green-700 dark:text-green-500">
                        {selectedBarber?.name} is available from <span className="font-bold">{currentDayAvailability.start_time.substring(0, 5)}</span> to <span className="font-bold">{currentDayAvailability.end_time.substring(0, 5)}</span>
                      </p>
                    </div>
                  ) : selectedDate && (
                    <div className="p-4 bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30 rounded-lg text-sm">
                      <p className="text-red-700 dark:text-red-500 font-medium">
                        {selectedBarber?.name} does not work on this day.
                      </p>
                    </div>
                  )}
        ...
        {step === 4 && (
          <div className="text-center space-y-6 py-12">
            ...
            <p className="text-muted-foreground max-w-md mx-auto text-lg">
              Your appointment with <strong className="text-foreground">{selectedBarber?.name}</strong> for a <strong className="text-foreground">{selectedService?.name}</strong> on <strong className="text-foreground">{selectedDate}</strong> at <strong className="text-foreground">{selectedTime}</strong> has been successfully scheduled.
            </p>
            <div className="pt-8">
