                    ...
                    <CardContent className="p-6 flex flex-col justify-between h-full">
                      <div className="mb-4">
                        ...
                        <div className="flex items-center text-muted-foreground text-sm mt-2 gap-4">
                          <span className="flex items-center gap-1">
                            <Clock className="w-4 h-4 ml-1 mr-1" /> {service.duration} {t('common.mins')}
                          </span>
                        </div>
                      </div>
                      <div className="flex items-center justify-between mt-4 pt-4 border-t border-border/50">
                        ...
                        <Button variant="ghost" size="sm" className="group">
                          {t('common.select')} <ArrowRight className="w-4 h-4 ml-2 mr-2 group-hover:translate-x-1 rtl:group-hover:-translate-x-1 transition-transform" />
                        </Button>
                      </div>
                    </CardContent>
                  </Card>
          ...
          <div className="space-y-4">
            <div className="flex items-center mb-6">
              <Button variant="ghost" size="sm" onClick={() => setStep(1)}>
                <ArrowLeft className="w-4 h-4 ml-2 mr-2 rtl:rotate-180" /> {t('common.back')}
              </Button>
              <h2 className="text-2xl font-bold flex-1 text-center pr-12">{t('wizard.select_barber')}</h2>
            </div>
            {isLoadingBarbers ? (
              <div className="flex justify-center p-12"><Loader2 className="w-8 h-8 animate-spin text-primary" /></div>
            ) : (
        ...
        {step === 3 && (
          <div className="space-y-6">
            <div className="flex items-center mb-6">
              ...
              <h2 className="text-2xl font-bold flex-1 text-center pr-12">{t('wizard.select_date_time')}</h2>
            </div>
                ...
                <div className="grid gap-6 sm:grid-cols-2">
                  <div>
                    ...
                    <div className="relative">
                      <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground w-4 h-4 ml-2" />
                      <Input 
                        type="date" 
                        value={selectedDate} 
                        onChange={(e) => {
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <Clock className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground w-4 h-4 ml-2" />
                      <Input 
                        type="time" 
                        value={selectedTime || ''} 
            ...
            <div className="flex justify-end pt-4">
              ...
              >
                {isCreating ? <Loader2 className="w-4 h-4 ml-2 mr-2 animate-spin" /> : null}
